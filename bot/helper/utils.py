...
async def add_task(message: Message):
    try:
      ...
      await msg.edit_text("```Encoding video...```")
      new_file = await encode(filepath)
      if new_file:
        await msg.edit_text("```Video Encoded, getting metadata...```")
        ...
        os.remove(new_file)
        os.remove(thumb)
        await msg.edit_text("```Video Encoded to x265```")
      else:
        await msg.edit_text("```Something wents wrong while encoding your file. Make sure it is not already in HEVC format.```")
        os.remove(filepath)
